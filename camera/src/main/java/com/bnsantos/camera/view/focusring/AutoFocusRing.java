...
import com.bnsantos.camera.view.InterpolateUtils;
...
class AutoFocusRing extends FocusRingRenderer {
  private static final String TAG = new String("AutoFocusRing");
  ...
  public void draw(long t, long dt, Canvas canvas) {
    ...
    if (mFocusState == FocusState.STATE_ENTER) {
      ...
      ringAlpha = (int) InterpolateUtils.lerp(0, 255, mEnterOpacityCurve.valueAt(rFade));
      ...
      float rFade = InterpolateUtils.unitRatio(t, mHardExitStartMillis, mHardExitDurationMillis);
      ringAlpha = (int) InterpolateUtils.lerp(255, 0, mExitOpacityCurve.valueAt(rFade));
